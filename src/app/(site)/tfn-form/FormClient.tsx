...
import { isPlausiblePhone, isValidEmail, isPlausibleDob, dobInputRange } from '@/lib/validate'
...
export function FormClient({ defaultLang = 'en' }: { defaultLang?: FormLang } = {}) {
  ...
  const validate = () => {
    ...
    const reqMsg = T('required')
    if (!firstName.trim()) e.firstName = reqMsg
    if (!lastName.trim())  e.lastName  = reqMsg
    if (!country.trim())   e.country   = reqMsg
    if (!passport.trim())  e.passport  = reqMsg
    if (!email.trim())     e.email     = reqMsg
    else if (!isValidEmail(email)) e.email = T('invalidEmail')
    if (!dob.trim())       e.dob       = reqMsg
    else if (!isPlausibleDob(dob)) e.dob = T('invalidDob')
    if (!whatsapp.trim())  e.whatsapp  = reqMsg
    else if (!isPlausiblePhone(whatsapp)) e.whatsapp = T('invalidPhone')
    if (!auPhone.trim())   e.auPhone   = reqMsg
    if (!gender)           e.gender    = reqMsg
    if (!marital)          e.marital   = reqMsg
    if (!address.trim())   e.address   = reqMsg
    if (!howHeard.trim())  e.howHeard  = reqMsg
    if (!selfie.file)      e.selfie    = reqMsg
    ...
    <div className="form-page-wrap">
      <div className="form-card">
        <div className="form-header">
          ...
          <h1 className="form-title">{T('titleTFN')}</h1>
        ...
        <form onSubmit={handleSubmit} noValidate>
              ...
              <ul style={{margin:'6px 0 0',paddingLeft:'18px'}}>
                {(Object.entries(errors) as [string, string][]).filter(([,v]) => v).map(([k, v]) => (
                  <li key={k} style={{fontSize:'12px',marginBottom:'2px'}}>{v === T('required') ? `${({
                    firstName:T('firstName'),lastName:T('lastName'),country:T('country'),
                    passport:T('passport'),email:T('email'),dob:T('dob'),
                    whatsapp:T('whatsapp'),auPhone:T('auPhone'),gender:T('gender'),
                    marital:T('marital'),address:T('address'),selfie:T('selfieWithPassport')
          ...
          <button type="submit" className="submit-btn" disabled={loading}>
            {loading ? <span className="btn-loading"><svg className="spin" width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="white" strokeWidth="2.5" strokeDasharray="40" strokeDashoffset="10"/></svg>{T('submitting')}</span> : T('submitTFN')}
          </button>
...
const css = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .form-input { display: block; width: 100%; padding: 12px 14px; font-size: 14px; font-family: inherit; color: #080F0D; background: #F5F9F7; border: 1.5px solid #CDE3DB; border-radius: 12px; outline: none; transition: border-color .15s; -webkit-appearance: none; }
  ...
  .radio-pill { display: inline-flex; align-items: center; padding: 9px 18px; border-radius: 100px; border: 1.5px solid #CDE3DB; font-size: 13px; font-weight: 500; color: #587066; cursor: pointer; transition: all .15s; background: #F5F9F7; }
  .radio-pill.selected { background: #0B5240; border-color: #0B5240; color: #fff; font-weight: 600; }
  .declaration-box { background: #F5F9F7; border: 1.5px solid #CDE3DB; border-radius: 14px; padding: 16px; }
