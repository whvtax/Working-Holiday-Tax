...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await auditLog('client.view', params.id)
  ...
  try {
    ...
    if (body.action === 'update') {
      ...
      try {
        ...
        const { getClientById } = await import('@/lib/db')
        const client = await getClientById(params.id)
        await auditLog('client.update', params.id)
        return NextResponse.json({ ok:true, client })
      } catch {
    ...
    if (body.action === 'clear') {
      ...
      await auditLog('client.clear', params.id)
      return NextResponse.json({ ok:true })
    }
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // 1. Delete all blob files from storage FIRST (before DB rows are gone)
  try {
    const { sql: dbSql } = await import('@vercel/postgres')
    const { del } = await import('@vercel/blob')
    const { rows } = await dbSql`
      SELECT blob_url FROM crm_client_files WHERE client_id = ${params.id}
    `
    // Delete each blob from Vercel Blob storage
    for (const row of rows) {
      try { await del(row.blob_url) } catch (blobErr) {
        console.error('[DELETE client] blob delete failed:', row.blob_url, blobErr)
      }
    }
    // Delete all file records from DB
    await dbSql`DELETE FROM crm_client_files WHERE client_id = ${params.id}`
  } catch (fileErr) {
    console.error('[DELETE client] file cleanup error:', fileErr)
    // Non-fatal — continue to delete the client record
  }

  // 2. Delete client record from DB
  try { const { deleteClient } = await import('@/lib/db'); await deleteClient(params.id) } catch {}
  await auditLog('client.delete', params.id)
  return NextResponse.json({ ok:true })
}
