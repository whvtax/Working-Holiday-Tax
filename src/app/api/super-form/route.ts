...
export async function POST(req: NextRequest) {
  try {
    ...
    const fileUrls = await uploadFiles([selfieFile], `super-form/${clientId}`)
    ...
    console.log('New super form submitted:', fullName, '| files:', fileUrls.length)
    return NextResponse.json({ ok: true })
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[super-form] FAILED:', msg, err)
