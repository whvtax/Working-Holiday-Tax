...
export async function POST(req: NextRequest) {
  try {
    ...
    await createTask({
      ...
      whatsapp,
      email,
      country:     sanitiseShort(formData.get('passportCountry') ?? formData.get('country')),
      dob:         sanitiseShort(formData.get('dob')),
      taxYear:     '',
      address:     sanitiseField(formData.get('auAddress') ?? formData.get('address')),
      ...
      auPhone:     sanitiseShort(formData.get('auPhone') ?? formData.get('smsPhone')),
