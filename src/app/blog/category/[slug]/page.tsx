...
/* ── The category introduction ────────────────────────────────────────────
   ...
   Two paragraphs per category, written to stand on their own without the grid
   below them: what this category actually covers, and who it is for, ending on
   what decides the answer rather than on how to do it yourself. Then one link
   to the service page that owns the cluster, with anchor text that says what
   the page is.
...
const CATEGORY_INTRO: Record<string, CategoryIntro> = {
  ...
  'abn': {
    paragraphs: [
      ...
      'They are for anyone who has been asked to get an ABN by a farm, a delivery platform or a hospitality operator, and for anyone who already has one and is not sure what it changed. Two facts do most of the work. Nothing is withheld from an ABN payment and no super is paid on it, so the tax arrives in one piece at the end of the year rather than a little at a time. And the arrangement is classified on how the work actually runs, not on the word used for it, which is why being moved from payroll to invoicing for the same job, the same hours and the same supervision is the first thing worth reading about here.',
    ],
  ...
  'super': {
    paragraphs: [
      ...
      'They are for anyone at the end of a working holiday, and in particular for anyone who worked casually across several employers and suspects the money went to more than one fund. Two things decide how much comes back and how long it takes. Whether every account sitting under your TFN was found before the claim went in, because a claim only empties the funds you name. And when the claim is lodged relative to your visa ceasing. The payment itself is taxed at 65 per cent for working holiday makers, which is high, is set by law and cannot be planned around. The part that is worth getting right is not leaving an account behind.',
    ],
  ...
  'work-rights': {
    paragraphs: [
      ...
      'They are written for somebody in the middle of a season rather than at the end of one, and most of what is here is not a tax question. It is not treated as one. It sits on this site because the same year shows up twice: an employer who underpaid you, paid you cash, or never paid super is also an employer whose income statement will not match what you were actually paid, and that reappears the moment a return is lodged.',
    ],
    service: { path: '/tax-return', label: 'What we go through on every tax return' },
  },
  'medicare-and-other': {
    paragraphs: [
      ...
      'It is a small category with a very concrete number attached. On $25,000 of earnings the levy is $500. Claiming the exemption is not automatic and the ATO will not apply it for you: it has to be claimed on the return, and to claim it you need a Medicare Entitlement Statement from Services Australia, which must be applied for and commonly takes weeks to arrive. Your passport decides whether the exemption is available to you at all. Those two facts, the lead time and the passport, are what these guides keep returning to.',
    ],
