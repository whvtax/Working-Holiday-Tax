...
function calc(inc:number, wit:number, visa:string): Result {
  ...
  const d = wit - tax
  if (d > 0)  return { label:'Estimated refund', amount:`$${Math.round(d).toLocaleString()}`,           sub: visa==='whm' ? 'Working Holiday Maker rate applied' : 'Australian resident rates applied', pct:Math.min(100,(d/wit)*100), owing:false }
  if (d < 0)  return { label:'Tax owing',        amount:`$${Math.round(Math.abs(d)).toLocaleString()}`, sub:'You may owe additional tax. Contact us to discuss your options.',                         pct:100,                        owing:true  }
...
export function CalculatorClient() {
  ...
  const run = () => {
    const i = parseFloat(income)||0, w = parseFloat(withheld)||0
    if (!i||!w||!visa) { setErr('Please fill in all three fields.'); return }
    setErr(''); setResult(calc(i,w,visa))
  }
  ...
  return (
    <>
      ...
      <div className="relative overflow-hidden bg-ink-2 pt-[68px] grid-bg">
        <div className="absolute pointer-events-none" style={{top:'-25%',right:'-15%',width:'65%',paddingBottom:'65%',borderRadius:'50%',background:'radial-gradient(circle,rgba(11,82,64,.6) 0%,transparent 68%)'}}/>
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-14 pt-24 pb-24 relative z-10 text-center">
          <nav className="flex items-center justify-center gap-2 text-[12px] text-white/30 mb-6">
            <a href="/" className="hover:text-white/60 transition-colors">Home</a>
            <span className="text-white/15">/</span><span>Calculator</span>
          </nav>
          <div className="inline-flex items-center gap-2 bg-forest-300/10 border border-forest-300/22 rounded-full px-3.5 py-1.5 mb-6">
            <span className="w-[6px] h-[6px] rounded-full bg-forest-300"/>
            ...
            <div className="bg-forest-50 border border-border rounded-[22px] p-8 md:p-10">
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select id="cv" value={visa} onChange={e=>setVisa(e.target.value)}
                    ...
                    <svg className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" width="12" height="8" viewBox="0 0 12 8" fill="none"><path d="M1 1l5 5 5-5" stroke="#587066" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </div>
                </div>
              ...
              {result && (
                <div className="border-t border-border mt-8 pt-8">
                  ...
                  <p className="font-serif font-black text-[52px] tracking-[-2px] leading-none mb-1.5" style={{color:result.owing?'#E9A020':'#0B5240'}}>{result.amount}</p>
                  <p className="text-[12px] text-subtle mb-5">{result.sub}</p>
                  <div className="h-[2.5px] bg-border rounded-sm overflow-hidden mb-7">
                  ...
                  <a href={WA_URL} target="_blank" rel="noopener noreferrer"
                    ...
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="9.5" fill="rgba(255,255,255,.14)"/><path d="M10 3C6.1 3 3 6.1 3 10c0 1.3.36 2.52.98 3.56L3 17l3.5-.96C7.5 16.67 8.73 17 10 17c3.9 0 7-3.1 7-7s-3.1-7-7-7z" stroke="white" strokeWidth="0.5" fill="white"/><path d="M13.6 13.1c-.13.35-.84.7-1.16.73-.3.04-.6.16-2-.52-1.7-.8-2.8-2.53-2.88-2.65-.08-.12-.72-1.07-.72-2.04s.52-1.44.7-1.64c.18-.2.4-.24.53-.24h.38c.12 0 .27 0 .4.32.13.32.46 1.37.5 1.47.04.1.06.22 0 .35l-.36.48c-.1.12-.2.25-.08.48.12.23.52.94 1.1 1.46.58.52 1.08.74 1.3.83.22.1.3.08.4-.05l.37-.52c.1-.14.22-.12.36-.06.14.06.94.52 1.1.62.16.1.27.15.3.23.04.32-.08.9-.2 1.2z" fill="rgba(0,0,0,0.1)"/></svg>
