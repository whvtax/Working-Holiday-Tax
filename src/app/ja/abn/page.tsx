...
const FAQS = [
  {
    ...
    answer:
      'ご自身でできますし、1年間がすべて給与だけだったのなら簡単です。ABNが入った時点で簡単ではなくなります。給与と請求収入の境界線をどこに引くのか、そもそも請負だったのか、どの費用を請求収入から差し引き、その裏づけに何を残すのか、そしてGSTの登録が必要だったのか。いずれも入力欄ではなくあなたの1年についての判断で、ここを誤ることが申告のやり直しになる最も多い原因です。',
  },
  {
    question: 'ABNがあると、タックスリターンはどう変わりますか。',
    answer:
      '給与は税金が引かれた状態で支払われ、雇用主がインカムステートメントをATOに提出するため、この部分はおおむね自動的に整合します。ABNで請求した収入は税引き前で入金され、事業収入として申告し、証拠を示せる費用の分だけ圧縮できます。そこにGSTの扱いと、「本当に請負だったのか」という問いが重なり、それでも両方が1つの申告に載ります。',
  },
  {
    question: '普通のシフト勤務なのに雇用主からABNを求められました。正しいですか。',
    answer:
      'たいていは正しくありません。登録する前に確認する価値があります。シフトを組まれ、作業方法を指示され、道具を支給され、早く帰らされることがあるなら、書類上の名目がどうであれ、実態としては被雇用者として扱われています。ABNに切り替えることでコストはあなたに移ります。源泉徴収なし、スーパーアニュエーションなし、労災保険なし、最低賃金や割増賃金の保護もなしです。何かを承諾する前に求人の詳細をお送りください。',
  },
  {
    question: 'ワーキングホリデービザでGSTの登録は必要ですか。',
    answer:
      '年間売上が75,000ドルに達する場合、またはライドシェアを運転する場合だけです。ほとんどのワーキングホリデーメーカーはこの金額に届きません。UberやDiDiなどタクシー・配車サービスを提供する方は、どれだけ少額であっても最初の1回の乗車からGST登録が必要です。フードデリバリーや宅配の配達員にこのルールは適用されず、75,000ドルの基準になります。登録すると四半期ごとのBAS（事業活動報告）の提出義務が付いてくるため、軽い気持ちで選ぶものではありません。',
  },
  {
    question: 'TFNとABNの両方を持つことはできますか。',
    answer:
      'できますし、請負で働くワーキングホリデーメーカーの多くは最終的に両方を持っています。タックスファイルナンバーは被雇用者としてのあなたを、ABNは個人事業主としてのあなたをカバーします。同じ年度に両方で稼いでも何も問題ありません。ただし申告が2つになるわけではありません。1つの申告が1つの年度をカバーし、その中で両方を報告します。ABNの申請はTFNと照合されるため、TFNが先に必要です。',
  },
  {
    question: 'ABNの必要経費として何が控除できますか。',
    answer:
      'その収入を得るためにかかった費用です。私用と兼用のものは正直に按分します。配達員であれば、きちんと記録した走行距離、自転車や車の維持費、携帯電話と通信費、保険、装備、そして入金前にプラットフォームが差し引いた手数料が中心になります。建設現場の下請けであれば、工具、保護具、現場間の移動です。含まれないのは、オーストラリアへの渡航費、自宅から1か所の職場への通常の通勤、そして記録を示せないものすべてです。',
  },
  {
    question: 'オーストラリアを離れるとき、ABNはどうなりますか。',
    answer:
      '事業をやめた時点で取り消します。ABNが有効なままだと、ATOから見て事業を続けており、義務も続いている状態に見えるためです。取り消しても、働いた年度の申告義務がなくなるわけではなく、同じ期間の雇用収入から生じるスーパーアニュエーションの受け取りにも影響しません。帰国が近い場合は早めにお知らせください。取り消し、申告、スーパー申請の順番が重要になります。',
  },
]
      ...
      <section className="py-12 lg:py-16" style={{ background: '#F5F9F7' }}>
        <div className="max-w-[820px] mx-auto px-5 md:px-8 reveal">
          ...
          <p style={{ ...BODY, color: '#2A3C34', maxWidth: '44ch', marginBottom: '16px' }}>
            誰かが仕事の中身を見るまでは、登録すべきではありません。実態は雇用なのにABNにすると、スーパーアニュエーションが積み立てられず、税金も源泉徴収されず、労災保険の対象から外れ、最低賃金や割増賃金の枠組みの外に出て、そのすべてがあなたの負担になります。ファーム、飲食、清掃、建設現場ではよくあることで、「ここではこうするものだ」という形で提示されます。
          </p>
      ...
      <section className="py-12 lg:py-16 bg-white">
        <div className="max-w-[1000px] mx-auto px-5 md:px-8 reveal">
          ...
          <p style={{ ...BODY, color: '#4C6459', maxWidth: '44ch', marginBottom: '24px' }}>
            請負か雇用かの判断基準、個人事業主が控除できるもの、GSTが始まる線。
          </p>
