...
const MYGOV = [
  ...
  },
  {
    ...
    us: '裏づけがある場合にのみ申請します。後から問われたときに、申告が根拠として示せるのはその書類です。',
  },
]
...
const faqs = [
  ...
  },
  {
    ...
    answer:
      '多くは対象ですが全員ではありません。決め手はビザではなくパスポートで、イギリス、アイルランド、イタリアを含む相互医療協定11か国の国籍であれば通常メディケアの対象となり、その対象であること自体が免除を外します。日本は協定を結んでいないため、日本のワーキングホリデーメーカーは通常、免除を申請できます。',
  },
  {
    question: 'Medicare Entitlement Statementとは何ですか。必要ですか。',
    answer:
      'Medicare Entitlement Statementは、指定した期間にメディケアの対象ではなかったことをServices Australiaが証明する書類です。免除の根拠となる証拠であり、ATOから提示を求められることがあります。裏づけのないまま免除を申請するのではなく、この書類の手配もタックスリターンの一部として当社が対応します。',
  },
  {
    question: '1年のうち一部の期間だけ免除を受けられますか。',
    answer:
      'できますし、ワーキングホリデーの方の多くは1年まるごとではなく一部期間が正しい答えになります。免除は日数で計算されるため、11月に入国した場合は、メディケアの対象でなかった日数だけが免除されます。一部しか当てはまらないのに1年分を申請するのは、後からタックスリターンの修正につながる典型的な誤りです。',
  },
  {
    question: '旅行保険や民間の医療保険に入っていると扱いは変わりますか。',
    answer:
      '変わりません。メディケア税で問われるのは公的制度の対象資格という税務上の点であり、保険に入っているかどうかではないため、旅行保険も民間医療保険も影響しません。民間の入院保険が関係するのはMedicare Levy Surchargeという別の課税で、高所得の場合に適用され、ワーキングホリデーではほとんど関係しません。',
  },
  {
    question: '年の途中ですでに差し引かれていた場合はどうなりますか。',
    answer:
      '失われるものはありません。メディケア税はタックスリターンの査定時に計算されるもので、給与支払いのたびに徴収されるものではないため、年間に差し引かれていたのは一般的な源泉徴収です。正当な免除を申請すれば、査定からメディケア税が外れます。免除を入れずに提出した過去の年度も、対象であれば通常は修正申告できます。',
  },
]
      ...
      <section className="py-8 lg:py-11" style={{ background: '#fff' }}>
        <div className="max-w-[1280px] mx-auto px-5 md:px-8 lg:px-12">
          <div className="max-w-[680px] mx-auto">
            ...
            <p style={BODY}>
              それが既定だからです。メディケア税はATOがタックスリターンを査定する際に課税所得へ加算され、免除を申請しない限りそのまま残ります。手続きのどこにも、メディケアの対象だったかどうかを尋ねる項目はありません。その結果として多いのが、一度も使えなかった制度に1年分の所得の2%を払っている、という状態です。
            </p>
            <p style={BODY}>
              もう一方の、静かなほうの誤りが、裏づけとなる書類がないまま免除にチェックを入れてしまうケースです。その根拠はServices Australiaが発行するMedicare Entitlement Statementで、ATOとは別の役所への申請になります。これがないまま申請すると、1年後、もうオーストラリアにいない頃に問われて説明できない申告が残ります。
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="who-is-exempt" className="py-8 lg:py-11" style={{ background: '#F5F9F7' }}>
        <div className="max-w-[1280px] mx-auto px-5 md:px-8 lg:px-12">
          <div className="max-w-[680px] mx-auto">
            ...
            <p style={BODY}>
              メディケアの対象ではなかった方が原則として免除されます。ワーキングホリデービザの場合、それを決めるのはパスポートです。オーストラリアは11か国と相互医療協定を結んでいます。協定国の国籍の方は滞在中メディケアの対象となるのが一般的で、登録していなくても、一度も使っていなくても、その対象であること自体が免除を外します。判定は利用実績ではなく対象資格です。それ以外の国、日本も含めて、通常は対象外であり、該当する日数について免除を申請できます。
            </p>
          </div>
      ...
      <section className="py-8 lg:py-11" style={{ background: '#fff' }}>
        <div className="max-w-[1280px] mx-auto px-5 md:px-8 lg:px-12">
          <div className="max-w-[680px] mx-auto">
            ...
            <p style={BODY}>
              メディケアの対象だったかどうか、そして年のどの期間が対象外だったかを確認します。チェックを入れる作業ではなく、国籍、ビザ、日付から判断する内容です。
            </p>
