...
const MYGOV = [
  ...
  },
  {
    ...
    us: 'あなたの国とオーストラリアに相互医療協定があるかどうかがすべてです。日本は協定を結んでいないため、通常は免除を申請できます。',
  },
  {
    mygov: '免除にはMedicare Entitlement Statementが必要で、それは申告からではなく別の窓口で申請します。',
    us: '当社が証明書を申請し、結果を待ち、正しい年度で免除が反映されるように提出します。',
  },
  {
    mygov: '証明書があってもなくても、免除の入力欄はそこにあります。',
    us: '裏づけがある場合にのみ申請します。だからこそ後から問われても通ります。',
  },
]
...
const faqs = [
  {
    ...
    answer:
      '申告をご自身で提出することはできますし、提出は簡単な部分です。メディケア税が難しいのは、そもそもそれがあなたの負担ではなかったかもしれないことを、提出の流れの中で誰も教えてくれない点にあります。課税所得の2%で、税額確定時に既定で加算され、給与明細に独立した項目としては出てこないため、ほとんどの方が気づきません。負担すべきかどうかを決めるのはビザではなくパスポートで、あなたの国とオーストラリアに相互医療協定があるかどうかが基準です。免除はチェックを入れれば済むものではなく、Services Australiaに別途申請するMedicare Entitlement Statementが必要で、発行までに時間もかかります。この証明書の裏づけなしに免除を申請すると、後から照会される形になります。ご相談の際にパスポートの国籍と、その年の入国日と出国日をお知らせいただければ、対象かどうかをお伝えできます。',
  },
  {
    question: 'メディケア税の免除はいくらになりますか。',
    answer:
      'メディケア税は課税所得の2%です。収入25,000ドルなら約500ドル、50,000ドルなら約1,000ドルになります。給与明細に独立した項目として出てこないため、支払っていること自体にほとんどの方が気づきません。精算はタックスリターンで行われるため、そこで免除を申請することがお金を取り戻す方法になります。気づいていてもいなくても、同じお金です。',
  },
  {
    question: '417・462ビザなら誰でも免除されますか。',
    answer:
      '多くは対象ですが全員ではなく、決め手はビザではなくパスポートです。オーストラリアは11か国と相互医療協定を結んでおり、イギリス、アイルランド、イタリアなどが含まれます。協定国の国籍であれば通常メディケアの対象となり、その対象であること自体が免除を外します。登録したことがなく、一度も使っていなくても同じです。日本は協定を結んでいないため、日本のワーキングホリデーメーカーは通常、免除を申請できます。',
  },
  {
    question: 'Medicare Entitlement Statementとは何ですか。必要ですか。',
    answer:
      'Medicare Entitlement Statementは、指定した期間にメディケアの対象ではなかったことをServices Australiaが証明する書類です。免除の根拠となる証拠であり、ATOから提示を求められることがあります。この書類なしに免除を申請すると、裏づけのない申告になります。タックスリターンとは別の役所への別の申請で、発行にも時間がかかるため、免除そのものを諦めてしまう方が多いのが実情です。',
  },
  {
    question: '1年のうち一部の期間だけ免除を受けられますか。',
    answer:
      'できますし、ワーキングホリデーの方の多くは1年まるごとではなく一部期間が正しい答えになります。免除は日数で計算されるため、11月に入国した場合や、途中で状況が変わった場合は、メディケアの対象でなかった日数だけが免除されます。一部しか当てはまらないのに1年分を申請するのは、後からタックスリターンの修正につながる典型的な誤りです。',
  },
  {
    question: '旅行保険や民間の医療保険に入っていると扱いは変わりますか。',
    answer:
      '変わりません。旅行保険や民間医療保険はメディケアとは別の制度で、メディケア税にも免除にも影響しません。メディケア税で問われるのは公的制度の対象資格という税務上の点であり、保険に入っているかどうかではありません。民間の入院保険が関係するのはMedicare Levy Surchargeという別の課税で、高所得の場合に適用され、ワーキングホリデーではほとんど関係しません。',
  },
  {
    question: '年の途中ですでに差し引かれていた場合はどうなりますか。',
    answer:
      '失われるものはありません。メディケア税はタックスリターンの査定時に計算されるもので、給与支払いのたびに徴収されるものではありません。年間に差し引かれていたのはメディケア税そのものではなく、一般的な源泉徴収です。正当な免除を申請すれば査定からメディケア税が外れ、還付が増えるか、納税額が減ります。免除を入れずに提出した過去の年度も、対象であれば通常は修正申告できます。',
  },
]
        ...
        <div className="max-w-[1280px] mx-auto px-5 md:px-8 lg:px-12">
          <div className="max-w-[680px] mx-auto">
            ...
            <p style={{ ...BODY, color: '#4C6459', maxWidth: '42ch', marginBottom: '20px' }}>
              課税所得の2%、25,000ドル稼いだ年なら約500ドルです。給与明細には出てこないため、払っていること自体に気づきません。
            </p>
      ...
      <section className="py-8 lg:py-11" style={{ background: '#fff' }}>
        <div className="max-w-[1280px] mx-auto px-5 md:px-8 lg:px-12">
          <div className="max-w-[680px] mx-auto">
            ...
            <p style={BODY}>
              それが既定だからです。メディケア税はATOがタックスリターンを査定する際に課税所得へ加算され、免除を申請しない限りそのまま残ります。手続きのどこにも、メディケアの対象だったかどうかを尋ねる項目はなく、肝心なところで注意も出ません。その結果として最も多いのが、一度も使えなかった制度に1年分の所得の2%を払っている、という状態です。
            </p>
            <p style={BODY}>
              次に多いのは、静かにもっと厄介です。裏づけとなる書類がないまま免除にチェックを入れてしまうケースです。免除の根拠はServices Australiaが発行するMedicare Entitlement Statementで、別の役所への別の申請であり、発行にも時間がかかります。これがないまま申請すると、ATOに尋ねられたときに説明できない申告が残ります。表に出てくるのはたいてい1年後、もうオーストラリアにいない頃です。
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="who-is-exempt" className="py-8 lg:py-11" style={{ background: '#F5F9F7' }}>
        <div className="max-w-[1280px] mx-auto px-5 md:px-8 lg:px-12">
          <div className="max-w-[680px] mx-auto">
            ...
            <p style={BODY}>
              メディケアの対象ではなかった方が原則として免除されます。ワーキングホリデービザの場合、それを決めるのはパスポートです。オーストラリアは11か国と相互医療協定を結んでいます。協定国の国籍の方は滞在中メディケアの対象となるのが一般的で、登録していなくても、一度も使っていなくても、その対象であること自体が免除を外します。判定は利用実績ではなく対象資格だからです。それ以外の国、日本も含めて、通常は対象外であり、該当する日数について免除を申請できます。
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-[880px] mx-auto" style={{ marginTop: '20px' }}>
            <div className="rounded-2xl" style={{ padding: '20px', background: '#fff', border: '1.5px solid #C8EAE0' }}>
              ...
              <p style={{ fontSize: '14px', lineHeight: 1.8, color: '#2A3C34', marginBottom: '12px' }}>
                原則としてメディケアの対象となるため、通常は免除されません。二重に課されないよう正しく処理し、年の一部で状況が違っていなかったかを確認します。
              </p>
            ...
            <div className="rounded-2xl" style={{ padding: '20px', background: '#EAF6F1', border: '1.5px solid #C8EAE0' }}>
              ...
              <p style={{ fontSize: '14px', lineHeight: 1.8, color: '#2A3C34', marginBottom: '12px' }}>
                通常はメディケアの対象外のため、免除の対象になります。日本もドイツもこちらに入り、この2か国でこのページを読む方のかなりの割合を占めます。
              </p>
              <p style={{ fontSize: '13px', lineHeight: 1.8, color: '#4C6459' }}>
      ...
      <section className="py-8 lg:py-11" style={{ background: '#fff' }}>
        <div className="max-w-[1280px] mx-auto px-5 md:px-8 lg:px-12">
          <div className="max-w-[680px] mx-auto">
            ...
            <p style={BODY}>
              まず、メディケアの対象だったかどうか、そして年のどの期間が対象外だったかを確認します。これはチェックを入れる作業ではなく、国籍、ビザ、日付から判断する内容です。対象外だった場合はMedicare Entitlement Statementを当社が代理で申請し、正しい日数分の免除をタックスリターンに反映します。後から確認されても説明できる形にするためです。
            </p>
            <p style={BODY}>
              別料金の作業ではなく、タックスリターンの一部です。居住区分や職種ごとの控除と並べて確認します。
            </p>
