...
const MYGOV = [
  ...
  },
  {
    ...
    us: '金額が大きいのはたいていタックスリターンの側なので、両方を扱います。',
  },
]
  ...
  return (
    <>
      ...
      <section className="py-8 lg:py-11" style={{ background: '#fff' }}>
        <div className="max-w-[1280px] mx-auto px-5 md:px-8 lg:px-12">
          <div className="max-w-[680px] mx-auto">
            ...
            {/* 2つのラベルは以前8つのセルすべてに出ていた。スマートフォンでは
                同じ言葉が画面を8回下りてくることになる。ラベルは最初の行にだけ
                出す。デスクトップでは列見出し、スマートフォンでは凡例として読め、
                以降は背景色と「当社の場合」側の太さが区別を担う。表の文言は
                変えていない。 */}
      ...
      <Answer id="dasp-vs-leaving" heading="今受け取るか、オーストラリアに残しておくか。">
        ...
        <p style={BODY}>
          残す理由があるのは、オーストラリアに戻って定住して働く予定がある場合だけです。待っても65%は下がりません。
