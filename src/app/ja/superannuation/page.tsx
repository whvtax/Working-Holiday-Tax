...
const faqs = [
  {
    ...
    answer:
      'ファンドが1つで書類がそろっていれば、ご自身での申請が合理的です。実際には短期の仕事でスーパーが忘れた口座に分散し、申請はファンドを1つずつ、名前を挙げられるものしか扱えません。ご依頼の場合はTFNに紐づく口座をすべて洗い出すところから始めます。',
  },
  {
  ...
  },
  {
    ...
    answer:
      '65%は、417・462ビザを一度でも保持した人に法律で定められた税率です。ワーキングホリデービザを持ったことがない一時滞在者には、課税済み部分に35%が適用されます。最後のビザではなくビザの履歴で決まる仕組みで、代理人が下げられるものではありません。',
  },
  {
  ...
  },
  {
    ...
    answer:
      'どちらにも影響しません。DASPの税金は支払い前に差し引かれる最終的な課税で、タックスリターンに記載する必要はなく、将来のビザ申請にも関係しません。影響はスーパー口座が閉鎖されることだけで、再び働く場合は新しいファンドで積み立てが始まります。',
  },
]
  ...
  return (
    <>
      ...
      <Answer id="documents" heading="DASP申請にはどの書類が必要ですか。">
        ...
        <p style={BODY}>
          申請が止まりやすいのは認証（certification）です。残高が5,000ドル以上のファンドでは、パスポートとビザの認証済みコピーを求められるのが一般的で、日本からの手配には時間がかかります。事前に整えておくかどうかが、申請期間の分かれ目です。
        </p>
      </Answer>
      ...
      <Answer id="with-us" heading="自分で申請するか、当社に任せるか。" tint>
        ...
        <p style={BODY}>
          当社が引き受けるのは、そうならない部分です。ATO保有分も含めてTFNに紐づくすべての口座を探し、日本からの認証書類を手配し、返答が止まったファンドを追いかけます。ワーホリの税務だけを扱っているので、どれも初めてではありません。
        </p>
