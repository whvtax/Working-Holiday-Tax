...
export function PageHeader({ kicker = 'Service guide', title, titleEm, sub, breadcrumbs, cta }: Props) {
  return (
    <section className="page-header grid-bg relative overflow-hidden bg-ink-2">
      <div className="absolute pointer-events-none" style={{ top:'-30%', right:'-10%', width:'65%', paddingBottom:'65%', borderRadius:'50%', background:'radial-gradient(circle,rgba(11,82,64,.7) 0%,transparent 68%)' }} />
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-14 relative z-10">
        {breadcrumbs && (
          <nav className="flex items-center gap-2 text-[12px] text-white/30 mb-5 reveal">
            {breadcrumbs.map((b, i) => (
              <span key={i} className="flex items-center gap-2">
                {i > 0 && <span className="text-white/15">/</span>}
                {b.href ? <Link href={b.href} className="hover:text-white/60 transition-colors">{b.label}</Link> : <span>{b.label}</span>}
              </span>
            ))}
          </nav>
        )}
        ...
        {cta && (
          <div className="reveal delay-4">
            <a href={cta.href ?? WA_URL} target={cta.href ? '_self' : '_blank'} rel="noopener noreferrer"
